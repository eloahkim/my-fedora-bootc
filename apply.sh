...
IMAGE="localhost/fedora-mango-noctalia:44"
...
if command -v bootc >/dev/null 2>&1; then
  ...
  echo "==> Pronto. Reinicie (systemctl reboot) e escolha a sessao 'Mango' no ly."
  ...
  echo "       sudo rpm-ostree rebase ostree-unverified-registry:ghcr.io/<user>/fedora-mango-noctalia:44"
  exit 1
fi
